...
#
# Needs: SUPABASE_ACCESS_TOKEN_EZ (Management API), curl, jq, node.
...
SERVICE=$(curl -sf -m 60 "$API/api-keys?reveal=true" -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN_EZ" | jq -r '.[] | select(.name=="service_role") | .api_key')
...
sql() {
  local q; q=$(jq -cn --arg q "$1" '{query:$q}')
...
cleanup() {
  ...
  local left; left=$(sql "select count(*) as n from schools where slug like '%$TAG'" | jq -r '.[0].n')
  local users; users=$(sql "select count(*) as n from profiles where name = '$TAG'" | jq -r '.[0].n')
...
echo "== schema present"
COL=$(sql "select data_type from information_schema.columns where table_schema='public' and table_name='schools' and column_name='pending'" | jq -r '.[0].data_type // empty')
check "schools.pending exists (boolean)" '[ "$COL" = "boolean" ]' "got '$COL'"
FNS=$(sql "select string_agg(proname, ',' order by proname) as f from pg_proc where proname in ('suggest_school','approve_school')" | jq -r '.[0].f')
...
check "school row exists and is pending" '[ "$(echo "$ROW" | jq -r ".[0].pending")" = true ]' "$ROW"
check "display_name keeps casing, collapses whitespace" '[ "$(echo "$ROW" | jq -r ".[0].display_name")" = "Rice University $TAG" ]' "$ROW"
PS=$(sql "select school from profiles where id = '$USER_ID'" | jq -r '.[0].school')
check "profile.school assigned" '[ "$PS" = "rice-university-$TAG" ]' "got $PS"
N=$(curl -s -m 60 "$URL/rest/v1/schools?select=slug&slug=eq.rice-university-$TAG" -H "apikey: $ANON" -H "Authorization: Bearer $TOKEN" | jq 'length')
check "submitter can read own pending row via RLS" '[ "$N" = 1 ]' "got $N rows"
N=$(curl -s -m 60 "$URL/rest/v1/schools?select=slug&slug=eq.rice-university-$TAG" -H "apikey: $ANON" -H "Authorization: Bearer $ANON" | jq 'length')
...
check "roster still loads for a user with a pending school" '[ "$RC" = 200 ] && [ "$(jq -r "[.[] | select(.id == \"$USER_ID\")] | length" /tmp/pm-roster)" = 1 ]' "http $RC"
...
check "anon call inserted nothing" '[ "$(sql "select count(*) as n from schools where slug like '"'"'anon-school%'"'"'" | jq -r ".[0].n")" = 0 ]' ""
...
check "4th row not inserted" '[ "$(sql "select count(*) as n from schools where slug = '"'"'fourth-school-$TAG'"'"'" | jq -r ".[0].n")" = 0 ]' ""
...
P=$(sql "select pending from schools where slug = 'rice-university-$TAG'" | jq -r '.[0].pending')
...
P=$(sql "select pending from schools where slug = 'sql-editor-$TAG'" | jq -r '.[0].pending')
...
SEEDED=$(curl -s -m 60 "$URL/rest/v1/schools?select=slug&pending=eq.false&order=slug" -H "apikey: $ANON" -H "Authorization: Bearer $ANON" | jq -r '[.[].slug] | join(",")')
